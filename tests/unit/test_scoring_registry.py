...
def test_score_race_dispatches_to_legacy_calibrated_curve():
    scores = score_race(
        _race(),
        [_finisher("1", 3600)],
        model_version=COURSE_STANDARD_LEGACY_CALIBRATED_VERSION,
    )
    assert scores[0].score.scoring_version == COURSE_STANDARD_LEGACY_CALIBRATED_VERSION
