...
**Publishing**: when an organizer publishes a race, the moment and their confirmation that they organize it are recorded with the race, together with what the automatic check noted (`api/screening.py`: signs that a results file is invented or copied, in plain words) and any decision an admin took, with the admin's address and note. The organizer sees all of this on the race's review page; the public sees only whether the race is pending, verified, or not public. The OTRI admins receive an email for every publish naming the race, the organizer's address and the finisher count.
