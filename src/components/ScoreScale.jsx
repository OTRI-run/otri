...
export default function ScoreScale({ score, share, exponent, targetSeconds, timeForScore }) {
  ...
  return (
    <div className="mx-auto mt-5 w-full max-w-[340px] text-left">
      ...
      <p className="mt-1 text-[12px] leading-[1.45] text-rule">{level.blurb}</p>
      ...
      >
        <div className="pointer-events-none absolute -top-[22px] -translate-x-1/2 transition-[left] duration-300" style={{ left: position(score) }}>
          <span className="block rounded-[3px] bg-white px-1.5 py-0.5 font-mono text-[10px] font-bold leading-none text-ink shadow">{score}</span>
        ...
        <div className="relative mt-1 h-3 font-mono text-[8px] text-muted">
          {[300, 500, 700, 900].map((tick) => (
            <span key={tick} className="absolute -translate-x-1/2" style={{ left: position(tick) }}>{tick}</span>
          ))}
          <span className="absolute -translate-x-1/2 text-rule" style={{ left: position(1000) }}>1000</span>
        </div>
        <div className="mt-1 flex justify-between font-mono text-[8px] tracking-[.08em] text-muted">
          <span>BEGINNER</span>
          <span>WORLD RECORD ↑</span>
        </div>
      </div>

      <ul className="mt-4 space-y-1.5 border-t border-slate-700/70 pt-3 text-[12px] leading-[1.45] text-rule">
        {marathon && score < 1000 && (
          <li>
      ...
      <details className="group mt-3">
        ...
        <table className="mt-2 w-full border-collapse text-[11px]">
          <thead>
            <tr className="text-left font-mono text-[8px] tracking-[.06em] text-muted">
          ...
          <tbody>
            {LEVELS.map((band, index) => {
              ...
              return (
                <tr key={band.id} className={`border-t border-slate-700/50 ${band.id === level.id ? 'text-white' : 'text-muted'}`}>
                  <td className="py-1">
