...
export default function ColumnsRead({ columns, ignored = [], className = '' }) {
  ...
  return (
    <details className={`rounded-[3px] border border-rule bg-white px-4 py-3 text-sm ${className}`}>
      <summary className="cursor-pointer font-semibold text-ink">
        How your file was read <span className="font-normal text-muted">· {shown.length} column{shown.length === 1 ? '' : 's'} used{ignored.length ? `, ${ignored.length} left alone` : ''}</span>
      </summary>
      <dl className="mt-3 grid gap-x-6 gap-y-1.5 text-[13px] sm:grid-cols-2">
        {shown.map((field) => (
          <div key={field} className="flex min-w-0 items-baseline justify-between gap-3 border-b border-rule pb-1.5">
            <dt className="text-muted">{LABELS[field]}</dt>
            <dd className="min-w-0 truncate font-mono text-[12px] font-semibold text-ink" title={columns[field]}>{columns[field]}</dd>
          </div>
        ))}
      </dl>
      {!columns.rank && <p className="mt-3 text-xs text-muted">No position column: positions were worked out from the finish times.</p>}
      {ignored.length > 0 && (
