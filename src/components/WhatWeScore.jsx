...
const MARK = {
  ...
  no: ['No', 'bg-slate-100 text-muted border-rule'],
}
      ...
      <ul className="divide-y divide-slate-100">
        {WHAT_WE_SCORE.map(([state, kind, note]) => (
          <li key={kind} className="grid gap-x-3 gap-y-0.5 py-2.5 sm:grid-cols-[92px_minmax(0,1fr)]">
            ...
            <span className="min-w-0 text-[13px] leading-5 text-muted">
              <b className="text-ink">{kind}.</b> {note}
            </span>
          </li>
        ))}
      </ul>
      <p className="mt-2 border-t border-rule pt-3 text-xs leading-5 text-muted">{NOT_MEASURED}</p>
    </div>
  )
}
...
export default function WhatWeScore({ className = '' }) {
  return (
    <details className={`group rounded-[3px] border border-rule bg-white px-4 py-3 text-sm text-muted ${className}`}>
      <summary className="cursor-pointer list-none font-semibold text-ink">
        <span className="text-accent">Which races can OTRI score? Trail, road, vertical…</span>
      </summary>
