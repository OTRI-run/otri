        ...
        <table className="w-full min-w-[680px] text-left text-sm">
          ...
          <tbody>
            {filtered.slice(0, visible).map((row, index) => (
              <tr key={`${row.runner_id ?? row.bib_number ?? index}-${row.rank}-${index}`} className="border-b border-slate-100 last:border-0 odd:bg-white even:bg-slate-50/70 hover:bg-blue-50/50">
