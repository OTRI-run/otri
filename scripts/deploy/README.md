...
| 10 | `10-install-service.sh [service_user] [app_dir]` | root (sudo) | Installs or updates the `otri-api` systemd unit and restarts the API. Root's job on purpose: whoever may write a unit file decides which program runs as which user. `02-deploy-app.sh` compares the installed unit with the one this would write and asks for this script when they differ (rare: a new sandbox path, a different worker count). `--print` writes the unit to stdout without root. |
