...
const LABEL = 'font-mono text-[10px] font-medium tracking-[.1em] text-accent'
const H2 = 'text-[clamp(22px,3vw,30px)] font-bold tracking-[-.04em] text-ink'
...
function FileLink({ href, children }) {
  return (
    <a href={href} download className="inline-flex min-h-8 items-center gap-1.5 rounded-[3px] border border-rule bg-white px-2.5 font-mono text-[11px] font-semibold text-ink no-underline hover:border-blue-400 hover:text-accent">
...
function LogoCard({ logo }) {
  return (
    <figure className="m-0 flex min-w-0 flex-col overflow-hidden rounded-[3px] border border-rule bg-white">
      ...
      <figcaption className="flex flex-1 flex-col gap-3 border-t border-rule p-4">
        <div>
          <p className="text-sm font-semibold text-ink">{logo.title}</p>
          <p className="mt-0.5 text-[13px] leading-5 text-muted">{logo.note}</p>
        </div>
    ...
    <>
      <section className="border-b border-rule bg-[radial-gradient(circle_at_78%_28%,rgba(37,99,235,.12),transparent_30%),linear-gradient(180deg,#fff_0%,#f8fbff_100%)]">
        <div className={`${CONTAINER} py-12 sm:py-14`}>
          <div className={LABEL}>
            OPEN TRAIL RUNNING INDEX <span className="text-rule">·</span> MEDIA AND BRAND
          </div>
          <h1 className="mt-5 max-w-[760px] text-[clamp(34px,5vw,56px)] font-normal leading-[1.02] tracking-[-.06em] text-ink">
            Our logo,
            <br />
            <em className="not-italic text-accent">yours to use.</em>
          </h1>
          ...
          <div className="mt-7 flex flex-wrap items-center gap-3">
            <a href={file('otri-brand-kit.zip')} download className="inline-flex min-h-11 items-center gap-2 rounded-[3px] bg-accent px-5 text-sm font-semibold text-white no-underline  hover:bg-accent">
              <Download size={16} /> Download the whole kit <span className="font-mono text-[11px] font-medium text-blue-100">ZIP · 0.7 MB</span>
            </a>
            <a href="mailto:hello@otri.run?subject=Press" className="inline-flex min-h-11 items-center gap-2 rounded-[3px] border border-rule bg-white px-5 text-sm font-semibold text-ink no-underline hover:border-accent">
      ...
      <div className={`${CONTAINER} py-12`}>
        ...
        <section aria-labelledby="media-badge" className="mt-16 grid gap-8 lg:grid-cols-[1fr_1.1fr] lg:items-start">
          <div>
            ...
            <div className="mt-5 flex flex-wrap items-center gap-4">
              <span className="inline-flex rounded-[3px] border border-rule bg-white p-4">
                <img src={file('otri-badge-scored.svg')} alt="Scored with OTRI" height="32" style={{ height: 32 }} />
              </span>
              <span className="inline-flex rounded-[3px] bg-slate-700 p-4">
          ...
          <div className="min-w-0 overflow-hidden rounded-[3px] border border-slate-800 bg-ink">
            <div className="flex items-center justify-between border-b border-slate-800 px-4 py-2">
              <span className="font-mono text-[9px] uppercase tracking-[.08em] text-muted">HTML for your results page</span>
              <button type="button" onClick={() => copy('badge', BADGE_HTML)} className="inline-flex items-center gap-1 font-mono text-[10px] text-rule hover:text-white">
        ...
        <section aria-labelledby="media-space" className="mt-16">
          ...
          <div className="mt-6 grid gap-4 lg:grid-cols-2">
            <div className="rounded-[3px] border border-rule bg-white p-6">
              <div className="flex justify-center py-4">
                ...
                <div className="relative inline-block p-[26px] outline-dashed outline-1 outline-blue-300" style={{ background: 'repeating-linear-gradient(45deg,rgba(37,99,235,.07) 0 6px,transparent 6px 12px)' }}>
                  ...
                  <span className="absolute left-1/2 top-1 -translate-x-1/2 font-mono text-[9px] text-accent">½ mark</span>
                  <span className="absolute left-0.5 top-1/2 -translate-y-1/2 font-mono text-[9px] text-accent">½</span>
                </div>
              </div>
              <p className="mt-3 text-sm font-semibold text-ink">Clear space</p>
              <p className="mt-1 text-[13px] leading-6 text-muted">Keep at least half the height of the mark free on every side: no text, no other logo, no edge of the page. The files already carry a little of it.</p>
            </div>
            <div className="rounded-[3px] border border-rule bg-white p-6">
              <div className="flex items-end justify-center gap-10 py-4">
                <div className="text-center">
                  ...
                  <p className="mt-2 font-mono text-[10px] text-muted">120 px · 30 mm</p>
                </div>
                <div className="text-center">
                  <img src={file('otri-logo-compact.svg')} alt="" style={{ width: 64 }} className="mx-auto block" />
                  <p className="mt-2 font-mono text-[10px] text-muted">64 px · 16 mm</p>
                </div>
                <div className="text-center">
                  <img src={file('otri-mark.svg')} alt="" style={{ width: 24 }} className="mx-auto block" />
                  <p className="mt-2 font-mono text-[10px] text-muted">24 px · 6 mm</p>
                </div>
              </div>
              <p className="mt-3 text-sm font-semibold text-ink">Smallest sizes</p>
              <p className="mt-1 text-[13px] leading-6 text-muted">Below these the full name stops being readable: switch to the compact logo, then to the mark, instead of shrinking further.</p>
            </div>
          </div>
        </section>
        ...
        <section aria-labelledby="media-dont" className="mt-16">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {DONT.map((item) => (
              <div key={item.label} className="overflow-hidden rounded-[3px] border border-rule bg-white">
                ...
                <p className="border-t border-rule p-3 text-[13px] leading-5 text-slate-700">{item.label}</p>
              </div>
            ))}
          </div>
          <ul className="mt-6 grid gap-x-10 gap-y-2 text-sm leading-7 text-muted sm:grid-cols-2">
        ...
        <section aria-labelledby="media-colour" className="mt-16">
          ...
          <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
            {COLOURS.map((colour) => (
              <button
                ...
                className={`group flex min-h-[132px] flex-col justify-between rounded-[3px] p-4 text-left transition hover:-translate-y-0.5 ${colour.border ? 'border border-rule' : ''}`}
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            <div className="rounded-[3px] border border-rule bg-white p-6">
              <p className="text-[44px] font-normal leading-none tracking-[-.05em] text-ink" style={{ fontFamily: 'Inter, ui-sans-serif, system-ui, sans-serif' }}>Aa 1000</p>
              <p className="mt-4 text-sm font-semibold text-ink">Inter</p>
              <p className="mt-1 text-[13px] leading-6 text-muted">Bold for the wordmark and headlines, regular for text. Open licence (SIL OFL). The site itself uses your device’s own sans-serif, which is its closest relative.</p>
            </div>
            <div className="rounded-[3px] border border-rule bg-white p-6">
              <p className="font-mono text-[30px] font-bold leading-none tracking-[.06em] text-accent">04:12:37</p>
              <p className="mt-4 text-sm font-semibold text-ink">JetBrains Mono</p>
              <p className="mt-1 text-[13px] leading-6 text-muted">For the full name, for labels, and for every number that is a measurement: times, distances, scores. Open licence (SIL OFL).</p>
            </div>
          </div>
        </section>
        ...
        <section aria-labelledby="media-words" className="mt-16 grid gap-8 lg:grid-cols-[1fr_1.2fr]">
          ...
          <div className="rounded-[3px] border border-rule bg-white p-6">
            <div className="flex items-center justify-between gap-3">
              <p className="font-mono text-[10px] font-medium tracking-[.08em] text-muted">ABOUT OTRI · FREE TO QUOTE</p>
              <button type="button" onClick={() => copy('about', BOILERPLATE)} className="inline-flex items-center gap-1 rounded-[3px] border border-rule px-2.5 py-1 font-mono text-[11px] font-semibold text-ink hover:border-blue-400">
        ...
        <p className="mt-14 border-t border-rule pt-6 text-[13px] leading-6 text-muted">
          ...
          <a href="mailto:hello@otri.run" className="font-medium text-accent">hello@otri.run</a>.
        </p>
      </div>
    </>
  )
