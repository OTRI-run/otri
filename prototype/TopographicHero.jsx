...
const west = Array.from({ length: 16 }, (_, i) => contour(105, 300, 65 + i * 27, .6))
const east = Array.from({ length: 18 }, (_, i) => contour(1310, 420, 45 + i * 26, 2.1))
...
export default function TopographicHero() {
  ...
  useEffect(() => {
    ...
    const paint = () => {
      ...
      element.style.setProperty('--trail-hidden', `${38 * (1 - progress)}`)
    }
    const schedule = () => {
    ...
    <div ref={ref} className="otri-terrain" aria-hidden="true">
      <svg className="otri-terrain-layer otri-terrain-far" viewBox="0 0 1440 900" preserveAspectRatio="none" focusable="false">
        <g fill="none" stroke="currentColor" strokeWidth="1">
          {west.map((d, i) => <path key={i} d={d} vectorEffect="non-scaling-stroke" strokeWidth={i % 4 === 0 ? 2 : 1} />)}
        </g>
      </svg>
      <svg className="otri-terrain-layer otri-terrain-near" viewBox="0 0 1440 900" preserveAspectRatio="none" focusable="false">
        <g fill="none" stroke="currentColor">
          {east.map((d, i) => <path key={i} d={d} vectorEffect="non-scaling-stroke" strokeWidth={i % 4 === 0 ? 2 : 1} />)}
        </g>
        <path d={trail} fill="none" stroke="#2563eb" strokeOpacity=".18" strokeWidth="3" vectorEffect="non-scaling-stroke" />
        <path className="otri-terrain-trail" d={trail} pathLength="100" fill="none" stroke="#2563eb" strokeWidth="3" strokeLinecap="round" vectorEffect="non-scaling-stroke" />
      </svg>
