...
export function VerticalBadge({ className = '' }) {
  return (
    <span className={`inline-flex items-center rounded-full border border-blue-200 bg-wash px-2 py-0.5 font-mono text-[10px] tracking-[.08em] text-accent ${className}`}>
      VERTICAL
    </span>
  )
}
...
export default function RaceCard({ race }) {
  ...
  return (
    <a
      ...
      className="group block min-w-0 rounded-[3px] border border-rule bg-white p-5 text-left no-underline  transition hover:border-accent hover:"
    >
      <div className="flex items-center justify-between gap-2">
        <p className="truncate font-mono text-[10px] tracking-[.08em] text-accent">{race.event_date}</p>
      ...
      <h3 className="otri-fit mt-2 text-xl font-bold leading-6 tracking-[-.03em] text-ink">{race.event_name}</h3>
      ...
      <div className="mt-4 flex flex-wrap gap-x-4 gap-y-1 font-mono text-[11px] text-muted">
        ...
        {race.has_gpx && <span className="text-accent">VERIFIED COURSE</span>}
      </div>
      <div className="mt-4 flex items-center justify-between text-xs font-semibold text-accent">
        ...
        <span className="text-muted transition group-hover:text-accent">{race.is_published === false ? 'Course and details →' : 'Leaderboard →'}</span>
      </div>
    </a>
  )
}
