...
- Scoring is now pluggable: a new default "Course Standard" model (Minetti gradient-cost course demand + a sealed 0-1000 curve) scores runners purely on course and finish time, with no dependency on competitors; the previous field-relative model is kept selectable per race distance.
