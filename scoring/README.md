...
### Course Standard (`2.0.0-course-standard`) — default

`scoring/course_standard.py` + `scoring/course_demand.py`. Implements [`docs/methodology/OTRI-SCORE-SCALE-FINAL-V0.md`](../docs/methodology/OTRI-SCORE-SCALE-FINAL-V0.md) exactly — that document is the source of truth for the mechanics of this model; this README only summarizes it.
...
- **Curved scale, clipped to 0–1000.** `OFFICIAL_CURVE` uses `Q(S) = exp(A + B·S + C·S²)`, anchored at OTRI 200/500/1000 → 11.0/15.0/30.0 demand-km/hour (`ScoreCurve.curve_type == "exponential_quadratic"`). Unlike a plain logarithmic curve, equal score steps near the top of the scale require progressively larger performance-rate increases than the same steps near the middle — see the doc's §4–§7 for the rationale. The unclipped raw value (`otri_raw`) is always retained for audit. **1000 is a real, reachable score**, not an asymptotic ceiling.
