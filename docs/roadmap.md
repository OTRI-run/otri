...
- [x] **Pluggable scoring architecture (`scoring/registry.py`)** — more than one scoring algorithm can be selected per race distance (`Race.scoring_version`):
  - **Course Standard (default; today OTRI model 0.1.0, build id `0.9.0-course-standard-domain-gated`, see `methodology/0.1.0/OTRI-MODEL-0.1.0.md`)** — its V0.1 build implements the course-demand engine now specified in [`methodology/0.1.0/OTRI-MODEL-0.1.0.md`](methodology/0.1.0/OTRI-MODEL-0.1.0.md) §3: a Minetti gradient-cost course-demand engine over fixed 50 m GPX segments (`scoring/course_demand.py` — 50 m chosen over the spec's own 20 m suggestion after real-world testing showed 20 m segments could let short-but-real technical trail pitches average out past the model's evidenced domain; a distance-based, not point-count-based, elevation-denoising pipeline since real GPX tracks are often very unevenly sampled; and clamp-and-flag handling for grades outside the polynomial's evidenced ±45% domain), feeding a logarithmic score curve. The default anchors 500 at 3.5 demand-km/h and 1000 at 10.5 demand-km/h — a recalibration of the spec's own literal anchors (15.0/22.5 demand-km/h) after real-world testing showed those clustered realistic multi-hour trail finishes at 0 (they implied ~70-minute absolute finish times for 500 points regardless of course length). The spec-literal curve remains selectable as `1.0.0-course-standard` for spec-fidelity comparisons. Has **no competitor dependency at all**: a runner's score depends only on the course and their own finish time, never the field's winner/size/strength. Pre-race GPX predictions (`POST /gpx/analyze`) and post-race scores share the exact same formula (`scoring.estimator` / `scoring.course_standard.score_for_time`), with an exact inverse (`target_time_seconds`) for score→time planning.
