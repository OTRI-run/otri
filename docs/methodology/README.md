...
- **How a score is computed today, and how far to trust it?** [`v0.7/OTRI-DEM-GATED-MEASUREMENT.md`](v0.7/OTRI-DEM-GATED-MEASUREMENT.md) documents `0.7.0-course-standard-dem-gated`, the **current default model**. The curve is V0.6's; what changes is measurement: elevation from a pinned terrain dataset when configured, a point-spacing gate so tracks too sparse to measure the route are marked for review instead of scored short, and a confidence label (High/Low) with the reason attached to every score.
