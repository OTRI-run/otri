...
## [`0.1.1/`](0.1.1/) — the scoring model in production

- [`OTRI-MODEL-0.1.1.md`](0.1.1/OTRI-MODEL-0.1.1.md): **OTRI model 0.1.1**, in production since 2026-09-25, specified as a difference from 0.1.0: the curve exponent is 0.692 instead of 0.85 ([OEP-004](../governance/oep/OEP-004-curve-exponent-0-692.md)), and nothing else changed.
- [`HOW-OTRI-SCORES.md`](0.1.1/HOW-OTRI-SCORES.md): what that does to the numbers, in plain language.

## [`0.1.0/`](0.1.0/) — the model 0.1.1 is built on
...
The next public model gets its own folder with its own pair of pages; this folder stays as published, and everything 0.1.1 does not name is defined here. The eight development builds that led to 0.1.0 are listed in the specification's §14 with what each contributed; their individual papers were removed in September 2026 because everything the model uses is on that one page (every build id still runs in `scoring/registry.py`, and the papers are in git history: `git log --all -- docs/methodology/v0.8`).
